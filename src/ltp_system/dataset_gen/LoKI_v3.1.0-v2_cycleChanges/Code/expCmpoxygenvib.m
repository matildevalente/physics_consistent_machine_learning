...
inputFile1 = 'oxygenlpp_chem_setup_vib_exp_comp.in';
inputFile2 = 'oxygenlpp3_noflow_4_v0_vib.chem';
...
%         fprintf(currentFileID, '%s%.2f%s\n', line(1:19), vibpop(idx),'.txt');
    ...
    while ~feof(defaultFileID)
      line = fgetl(defaultFileID);
      if strncmp(line, 'O(3P) + wall -> 0.5O2(X,v=0)                    | multicomponentTransportChantry   | x                               |', 84)
        fprintf(currentFileID, 'O(3P) + wall -> 0.5O2(X,v=0)                    | multicomponentTransportChantry   |  %.16e | \n' , lossprob(idx));
  ...
  movefile(['Output' filesep 'outputO2'],['Output' filesep 'ISTref_O2bquenching_Herz_kutasiO2b_vib' sprintf('%.1f', p(idx)./133.332) 'Torr_' sprintf('%.1f', targetI*1000) 'mA']);
