function [rateCoeff, dependent] = multicomponentTransportChantrySurface(~, densitiesAll, ~, reactionArray, ...
  reactionID, stateArray, workCond, ~, rateCoeffParams)
  % multicomponentTransportChantrySurface evaluates the transport rate coefficient of a particular species interacting 
  % with the wall, taking into account both the multicomponent diffusion time (using a Blanc's law) and the wall 
  % reaction time assuming a flux reaching the wall (with a certain wall recombination/deactivation probability). 
  % Additionally, the previous rate coefficient is modified to accomodate for the collision of the volume species with 
  % ONE surface species (dividing by surfaceSiteDensity*AreaOverVolume).
