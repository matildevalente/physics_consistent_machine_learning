...
def _compute_mape_and_sem(targets, predictions):
  ...
  sem_value = mape_std / np.sqrt(n_values)
...
def compute_projection_results(config_model, w_matrix, dataset_name, data_preprocessed, models):
    ...
    results = {key: {"constraints": value, "mape": [], "sem": []} for key, value in constraint_combinations.items()}
    ...
    # Calculate MAPE and SEM here
    for i in range(len(data_preprocessed.output_features)):
        mape_value, sem_value = _compute_mape_and_sem(denormalized_test_targets[:, i], denormalized_model_predictions[:, i])
        results["model"]["mape"].append(mape_value)
    ...
    for constraint_key, constraints in constraint_combinations.items():
        ...
        for i in range(len(data_preprocessed.output_features)):
            ...
            # Calculate MAPE and SEM 
            mape_value, sem_value = _compute_mape_and_sem(actual_values, predicted_values)

            results[constraint_key]["mape"].append(mape_value)
            results[constraint_key]["sem"].append(sem_value)
    
    return results

# compute the 
def compute_residual(inputs_norm, predictions_norm, model_type, data_prep):
  ...
  # 4. Apply MAPE calculation
  # For P_model and P_calc
  ape = np.array(np.abs((P_model - P_calc) / P_calc) * 100)
  p_mape = np.mean(ape)
  p_sem = np.std(ape, ddof=1) / np.sqrt(len(ape))
  p_abs_err = np.mean(np.array(np.abs(P_model - P_calc)))

  # For I_calc and I_model
  ape = np.array(np.abs((I_calc - I_model) / I_calc) * 100)
  i_mape = np.mean(ape)
  i_sem = np.std(ape, ddof=1) / np.sqrt(len(ape))
  i_abs_err = np.mean(np.array(np.abs(I_model - I_calc)))

  # For ne_model and ne_calc
  ape = np.array(np.abs((ne_model - ne_calc) / ne_calc) * 100)
  ne_mape = np.mean(ape)
  ne_sem = np.std(ape, ddof=1) / np.sqrt(len(ape))
  ne_abs_err = np.mean(np.array(np.abs(ne_model - ne_calc)))
...
def compute_mape_physical_laws(dataset_name, data_preprocessed, models, w_matrix, model_type):
  ...
  proj_nn_results = compute_residual(normalized_inputs, normalized_proj_predictions, model_type + "_proj", data_preprocessed)
  nn_results = compute_residual(normalized_inputs, normalized_model_predictions, model_type, data_preprocessed)

  return {**nn_results, **proj_nn_results}


def compute_mape_physical_laws_loki(dataset_name, data_preprocessed, w_matrix):
  ...
  # 5. compute mape and sem for the compliance with physical laws for NN and its projected predictions
  proj_loki_results = compute_residual(normalized_inputs, normalized_proj_targets, "loki_proj", data_preprocessed)
  loki_results = compute_residual(normalized_inputs, normalized_targets, "loki", data_preprocessed)
