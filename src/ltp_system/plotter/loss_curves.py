...
def loss_curves(config_model, config_plotting, losses_dict):
    ...
    plt.errorbar(
        ...
        label=r'Train Loss ($\mathcal{L_{\text{train}}}$)', markersize=2, color='#1f77b4', capsize=3, elinewidth=2, markeredgewidth=2, linewidth=2 
    )
    plt.errorbar(
        losses_dict['epoch'], losses_dict['losses_val_mean'], yerr= losses_dict['losses_val_err'],
        label=r'Validation Loss ($\mathcal{L_{\text{val}}}$)', markersize=2, color='#ff7f0e', capsize=3, elinewidth=2, markeredgewidth=2, linewidth=2 
    )

    if(model_name == 'PINN'):
        plt.errorbar(
            losses_dict['epoch'], losses_dict['losses_train_data_mean'], yerr= losses_dict['losses_train_data_err'],
            label=r'Weighted $\mathcal{L_{\text{data}}}$', markersize=2, color='#2ca02c', capsize=3, elinewidth=2, markeredgewidth=2, linewidth=2 
        )
        plt.errorbar(
            losses_dict['epoch'], losses_dict['losses_train_physics_mean'], yerr= losses_dict['losses_train_physics_err'],
            label=r'Weighted $\mathcal{L_{\text{physics}}}$', markersize=2, color='#d62728', capsize=3, elinewidth=2, markeredgewidth=2, linewidth=2 
        )

    plt.yscale('log')
    plt.xlabel('Epochs', fontsize=16, fontweight='bold')
    plt.ylabel(r'$\mathcal{L}_{\mathrm{MSE}}$', fontsize=20, fontweight='bold')
