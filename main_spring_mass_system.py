...
def main():
    try:
        ...
        if(config['pinn_model']['OPTIMIZE_ARC']):
            pinn_model_opt, pinn_opt_losses, config_pinn_opt = optimize_pinn_architecture(config, preprocessed_data)
        else:
            config_pinn_opt = load_config('configs/pinn_opt_config.yaml')
            #pinn_model_opt, pinn_opt_losses = get_trained_pinn(config_pinn_opt, preprocessed_data, checkpoint_dir=os.path.join('output', 'spring_mass_system', 'checkpoints', 'pinn_opt'))

        # Plot loss curves if specified in config
        plot_loss_curves(config, nn_losses, pinn_losses)

        # /// 5. GET MATRIX TO PERFORM PROJECTION ///
        #w_inv_cov_train,  w_inv_cov_trajectory = get_inverse_covariance_matrix(config,nn_model, device_nn, preprocessed_data)
        #print(w_inv_cov_train)
        ...
        plot_several_initial_conditions(config, preprocessed_data, nn_model, pinn_model, test_initial_conditions, w_inv_cov_matrix = torch.eye(4), n_time_steps = n_time_steps)
...
"""def evaluate_trajectories(config, nn_model, pinn_model, initial_states_list, n_time_steps, preprocessed_data):
    for initial_state in initial_states_list:
        df_target = get_target_trajectory(config, n_time_steps, initial_state)
        df_nn   = get_predicted_trajectory(config, nn_model,   n_time_steps, initial_state, preprocessed_data)
        df_pinn = get_predicted_trajectory(config, pinn_model, n_time_steps, initial_state, preprocessed_data)
        #df_proj = predict_trajectory_proj(config, nn_model, n_time_steps, initial_state, preprocessed_data)
"""
